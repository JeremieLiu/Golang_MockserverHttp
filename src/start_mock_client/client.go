...
func urlSubmissionResults(){
	...
	length := len(localSaveWord)
	log.Printf("headDate length:" ,length)
	if length<16 {
		log.Printf("data is not right for size whit :" ,length)
		return
	}
	headDate := localSaveWord[:16]
	log.Printf("headDate length:" ,len(headDate))
